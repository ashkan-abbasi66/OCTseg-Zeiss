...
def feature_fusion(out1, out2):
    ...
    out1_bg = torch.zeros([out1.shape[0], 1, out1.shape[2], out1.shape[3]]).to(device)
    out1_disc = torch.zeros([out1.shape[0], 1, out1.shape[2], out1.shape[3]]).to(device)
    out2_layer = torch.zeros([out2.shape[0], 9, out2.shape[2], out2.shape[3]]).to(device)
    out1_bg[:, 0, :, :] = out1[:, 0, :, :]
    out1_disc[:, 0, :, :] = out1[:, 2, :, :]
    out2_layer[:, :, :, :] = out2[:, 1:, :, :]
