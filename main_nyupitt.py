"""
# My notes
Dataset: `D:\OneDrive - Oregon Health & Science University\MY WORK\DATASETS\NYU-OCTseg-dataset\onh-3subsets`
`srun -u -p gpu -G 1 --mem-per-cpu=32GB --time=0-1`
- [ ] I ran into difficulty retraining the model on the upgraded Exacloud (ACC) while both training and testing can be done on my laptop. The source of error might be in the loss computation and related to type conversion. May be in the new version data types are not exchanged between PIL and PyTorch tensors as my code expected.

GOAL:
    Trains a one stage network (`OSMGUNet`) for only retinal layer segmentation.
    The DATASET is assumed to be the ONH subset of NYUPITT dataset.

- All images are stored at
    "E:/data/NYU-OCTseg-dataset/onh"

- "split_nyupitt_dataset.py" splits images and their labels into 3 subsets with specific directory structure.

-
-
A SIMPLE EXAMPLE USAGE FOR BOTH TRAIN AND TEST ON THE MGU DATASET:
    python main_nyupitt.py --name osmgunet --lr 0.001 --t t1 --batch-size 1 --epoch 50 -d data/NYU-OCTseg-dataset/onh-3subsets

TEST ON A NEW DATASET (locally)
--name osmgunet --lr 0.001 --t t1-nyupitt --batch-size 1 -d data/ --model-path logs/MGU/MGU-OCTseg-dataset/train/osmgunet_0.001_t1/model/model_best.pth.tar --local-test --predict
--name osmgunet --lr 0.001 --t t1-nyupitt --batch-size 1 -d data/NYUPITT-random-onh-bscans --model-path logs/OCTseg-MGU-Net-FINAL/onh-3subsets/train/osmgunet_0.001_t1/model/model_best.pth.tar --local-test --predict
=========================================
DATASET "data/NYU-OCTseg-dataset/onh"
=========================================
python main_nyupitt.py --name osmgunet --lr 0.001 --t nyupitt-mac --batch-size 1 --epoch 50 -d data/NYU-OCTseg-dataset/onh-3subsets


Note:

* `train_seg` sets up the training process and runs `train` and `eval` functions.
These two functions essentially perform the training on one batch. So, `train_seg` calls
them multiple times to finish one epoch.`

* `test_seg` loads the pretrained model and calls either `eval` or `eval_predict` functions.
 The latter does not need ground truth masks.
