...
def loss_builder2(class_num=11, ignore_index=255):
    criterion_2_1 = nn.NLLLoss(ignore_index=ignore_index)
    criterion_2_2 = DiceLoss(class_num=class_num)
...
class DiceLoss(nn.Module):
    ...
    def forward(self,input, target):
        ...
        self.smooth = 0.
        Dice = Variable(torch.Tensor([0]).float()).to(self.device)
        for i in range(1,self.class_num):
            ...
            if target_i.sum() == 0:
                dice = Variable(torch.Tensor([1]).float()).to(self.device)
            else:
